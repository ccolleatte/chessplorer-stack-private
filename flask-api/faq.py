# flask-api/faq.py 
# Version adaptée à votre système existant (/admin/questions/*)
...
class QuestionValidator:
    ...
    VALID_THEMES = [
        'Débutants', 'Intermédiaires', 'Avancés', 'Tactiques', 'Finales',
        'Ouvertures', 'Stratégie', 'Psychologie', 'Ressources', 'Général'
    ]
    
    @staticmethod
    def validate_question_data(data: dict) -> dict:
        """Validation adaptée à votre structure existante"""
        errors = []
        validated = {}
        ...
        if not texte:
            ...
            errors.append('Question trop courte (minimum 10 caractères)')
        else:
        ...
        try:
            ...
            errors.append('Niveaux doivent être des entiers')
...
# Décorateurs de sécurité (simplifiés pour votre usage)
def require_csrf():
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            # Version simplifiée - adaptez selon votre système d'auth
...
# ========================================
# 🎯 ROUTES ADAPTÉES À VOS URLs EXISTANTES
# ========================================
    ...
    try:
        ...
        cur.execute("""
            SELECT 
                ...
                CASE 
                    WHEN aliases::text IS DISTINCT FROM 'null' THEN 1
                    ELSE 0 
        ...
        for row in cur.fetchall():
            ...
            # Parsing JSON fields si nécessaire
...
def admin_questions_new():
    """Création question - utilise votre template existant"""
    if request.method == 'POST':
        ...
        try:
            ...
            # INSERT adapté à votre structure de table
            cur.execute("""
            ...
            logger.info(f"Question créée avec succès: ID {question_id}")
...
def admin_questions_edit(id):
    """Édition question - utilise votre template existant"""
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    ...
    if request.method == 'POST':
        ...
        if not validation['valid']:
            for error in validation['errors']:
                flash(error, 'error')
            # Récupération question pour réaffichage
        ...
        try:
            ...
            # UPDATE adapté à votre structure
            cur.execute("""
                UPDATE question SET
            ...
            conn.rollback()
            logger.error(f"Erreur mise à jour question {id}: {e}")
            flash('Erreur lors de la mise à jour', 'error')
        finally:
            cur.close()
            conn.close()

    # GET - Récupération pour édition
    cur.execute("SELECT * FROM question WHERE id = %s", (id,))
    question = cur.fetchone()
...
# ========================================
# 🔌 API POUR WORDPRESS (vos URLs existantes)
# ========================================
...
def api_chat_faq():
    ...
    try:
        ...
        cur.execute("""
            ...
            ORDER BY 
                CASE WHEN theme IN ('Débutants', 'Tactiques', 'Finales') THEN 1 ELSE 2 END,
                LENGTH(reponse) DESC
            LIMIT 3
        ...
        if not results:
            return jsonify({
                'success': True,
                'answer': 'Je n\'ai pas trouvé de réponse spécifique. Pouvez-vous reformuler votre question ?',
                'sources': [],
                'confidence': 0
            })
        
        # Utilise la meilleure réponse trouvée
        best_result = results[0]
        
        # Sélection réponse par niveau
        ...
        if best_result['reponses']:
            try:
                ...
                pass  # Garde la réponse générale
